...
import { CustomerScanner, useAddStamp } from '@/features/scan-customer';
...
function ProgramCard({ program, stamps, customerId, merchantId }: ProgramCardProps) {
  ...
  const activeStamps = stamps.filter(
    (s) => s.program_id === program.id && !s.redeemed,
  );
          ...
          <div
            ...
            className={`h-5 w-5 rounded-full border-2 transition-colors ${
              i < count
                ...
                : 'border-brown/20 bg-transparent'
            }`}
          />
        ))}
      </div>
    ...
    return (
      <div className="rounded-xl bg-red-50 p-5 text-center">
        ...
        <p className="mt-1 text-sm text-red-600">
          Ce QR code ne correspond à aucun client de votre commerce.
        </p>
        <button
          onClick={onReset}
  ...
  return (
    <div>
      <div className="mb-5 flex items-center gap-4 rounded-xl bg-brown p-5 text-cream shadow-sm">
        <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-orange text-lg font-bold">
          {customer.name.charAt(0).toUpperCase()}
        </div>
        <div className="min-w-0">
          <p className="truncate text-lg font-bold">{customer.name}</p>
          {customer.email && (
            <p className="truncate text-sm text-cream/60">{customer.email}</p>
          )}
      ...
      {programs.length === 0 ? (
        <p className="text-center text-sm text-brown/50">
          Aucun programme de fidélité actif pour ce commerce.
        </p>
      ) : (
  ...
  return (
    <div>
      ...
      <div className="mx-auto max-w-md">
        {customerId === null ? (
          <div>
            <p className="mb-4 text-center text-sm text-brown/60">
              Pointez la caméra vers le QR code de la carte du client
            </p>
            ...
            <div className="mt-4 rounded-xl bg-brown/5 p-4 text-center text-xs text-brown/50">
              Le QR code se trouve sur la carte digitale du client (page wallet ou Apple Wallet)
            </div>
          </div>
        ) : (
